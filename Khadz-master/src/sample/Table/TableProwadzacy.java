...
public class TableProwadzacy {
    ...
    public void setDodaj( String idprzedS, String idTytul, String idKatedry, String imie ,String nazwisko, String data, String pensja, String dzieci){
    ...
    public void setUsun(String idprzedS){
        try {
            Connection conn = ConnectTable.connectdb();
            conn.createStatement().executeUpdate("DELETE FROM `prowadzacy` WHERE id_prowadzacy = "+idprzedS);
            ResultSet rs = conn.createStatement().executeQuery("SELECT * FROM `prowadzacy`");
            while (rs.next()) {
                oblistProwadzacy.add(new ModelProwadzacy(rs.getString("id_prowadzacy"),
                        rs.getString("id_tytul"),
    ...
    public void setAktualizuj( String idprzedS, String tytul, String katedra, String imie, String nazwisko , String data, String pensja, String dzieci  ) {

        oblistProwadzacy.clear();
        try {
            Connection conn = ConnectTable.connectdb();
            conn.createStatement().executeUpdate("UPDATE `prowadzacy` SET  `id_tytul` = '" + tytul + "', `id_katedry` = '" + katedra + "', `imie_prowadzacy` = '" + imie + "' , `nazwisko_prowadzacy` = '" + nazwisko + "', `data_urodzenia_prowadzacy` = '" + data + "', `pensja` = '" + pensja + "', `ilosc_dzieci` = '" + dzieci + "' WHERE `id_prowadzacy`='" + idprzedS + "'");
            ResultSet rs = conn.createStatement().executeQuery("SELECT * FROM `prowadzacy`");
            while (rs.next()) {
                oblistProwadzacy.add(new ModelProwadzacy(rs.getString("id_prowadzacy"),
                        rs.getString("id_tytul"),
