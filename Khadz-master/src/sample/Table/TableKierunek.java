...
public class TableKierunek {
    ...
    public void setDodaj( String idprzedS, String idtypS, String nazwaS){
    ...
    public void setAktualizuj( String idprzedS, String idtypS, String nazwaS){

        oblistKierunek.clear();
        try {
            Connection conn = ConnectTable.connectdb();
            conn.createStatement().executeUpdate("UPDATE `kierunek` SET `id_wydzial`='"+idtypS+"',`nazwa_kierunku`= '"+nazwaS+"' WHERE `id_kierunek`='"+idprzedS+"'");
            ResultSet rs = conn.createStatement().executeQuery("SELECT * FROM `kierunek`");
            while (rs.next()){
                oblistKierunek.add(new ModelKierunek(rs.getString("id_kierunek"),
                        rs.getString("id_wydzial"),
