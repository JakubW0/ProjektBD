...
public class TableGrupa {
    ...
    public void setDodaj( String idprzedS,String nazwaS ){
    ...
    public void setAktualizuj( String idprzedS, String idtypS){

        oblistGrupa.clear();
        try {
            Connection conn = ConnectTable.connectdb();
            conn.createStatement().executeUpdate("UPDATE `gr_dziekan` SET `numer_grupy`='"+idtypS+"' WHERE `id_grupy`='"+idprzedS+"'");
            ResultSet rs = conn.createStatement().executeQuery("SELECT * FROM `gr_dziekan`");
            while (rs.next()){
                oblistGrupa.add(new ModelGrupa(rs.getString("id_grupy"),
                        rs.getString("numer_grupy")));
